...
// ✅ 从 scrpits/ui/shop 到 scrpits/items/ui/ItemDatabase.ts
...
// ✅ 工具函数：约束为 Component 子类（不要再单独 import Component 了）
...
export class ShopPanel extends Component {
    ...
    private _refreshNpcList() {
        ...
        for (const g of this._goods) {
            ...
            row.on(Node.EventType.TOUCH_END, () => this._tryBuy(g)); // 购买 1 个
    ...
    private _refreshPlayerList() {
        ...
        for (let i = 0; i < slots.length; i++) {
            ...
            row.on(Node.EventType.TOUCH_END, () => this._trySell(i)); // 出售 1 个
    ...
    private _toast(msg: string) { console.log('[ShopPanel]', msg); }
    ...
    private _buildUI() {
        ...
        add(left, UITransform).setContentSize(500, 520);
        const lw = add(left, Widget); lw.isAlignLeft = true; lw.left = 40; lw.isAlignTop = true; lw.top = 80;
        add(left, Layout).type = Layout.Type.VERTICAL; (left.getComponent(Layout)!).spacingY = 6;
        bg.addChild(left);
        ...
        add(right, UITransform).setContentSize(500, 520);
        const rw = add(right, Widget); rw.isAlignRight = true; rw.right = 40; rw.isAlignTop = true; rw.top = 80;
        add(right, Layout).type = Layout.Type.VERTICAL; (right.getComponent(Layout)!).spacingY = 6;
        bg.addChild(right);
    ...
    private _makeRow(text: string): Node {
        ...
        add(row, UITransform).setContentSize(500, 36);
        add(row, Button);
    ...
    private _makeLabel(text: string, fontSize: number): Label {
        ...
        add(n, UITransform).setContentSize(120, 30);
        const lb = add(n, Label); lb.string = text; lb.fontSize = fontSize; lb.lineHeight = fontSize + 8;
        return lb;
    }

    private _makeButton(text: string, onClick: () => void): Node {
        const n = new Node('button');
        add(n, UITransform).setContentSize(120, 40);
        add(n, Button);
