 ...
 * 商店窗口（左=NPC在售，右=玩家背包）
 * - 不移动场景背包；用 Canvas/InventoryPanel 作为模板克隆两份
 * - 打开时隐藏外层背包，关闭时恢复
 * - 整棵树重定向：把所有子组件的桥接/缓存都切到目标桥，然后 refresh
 */
import {
...
export class ShopWindow extends Component {
  ...
  static closeIfOpen() {
    ...
    if (inst && inst.node.isValid && inst.node.active) {
      inst._onClose();
    }
  }
  ...
  /* ---------- UI 外壳 ---------- */
  private _setUILayerDeep(n: Node) {
    n.layer = Layers.Enum.UI_2D;
    for (const c of n.children) this._setUILayerDeep(c);
  }
  private _build() {
    ...
    this._coinLb = coin.addComponent(Label);
    this._coinLb.string = '金币：0'; this._coinLb.fontSize = 22; this._coinLb.lineHeight = 26;
    ...
    this._setUILayerDeep(this.node);
  ...
  private _collectCells(root: Node): Node[] {
    const out: Node[] = [];
    const walk = (n: Node) => {
      if (n.name.startsWith('Cell_')) out.push(n);
  ...
  private _open(npcNode: Node | null, itemDBNode: Node | null, playerNode: Node | null) {
    ...
    this._setUILayerDeep(this._leftInvNode);
    ...
    this._setUILayerDeep(this._rightInvNode);
