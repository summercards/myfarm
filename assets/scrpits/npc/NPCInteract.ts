/*  NPCInteract.ts（R键 + 调试日志 + 触发器离开/距离超出自动关闭）
...
import { ShopWindow } from '../ui/shop/ShopWindow';   // ★ 新增：用于强制关闭商店
...
export class NPCInteract extends Component {
  ...
  onLoad() {
    ...
    // 尝试自动查找玩家（找不到也没关系）
    if (!this.player) this.player = find('play') || find('Player') || null;
  }
  ...
  onDisable() {
    ...
    ShopWindow.closeIfOpen();  // ★ 关闭商店
  }

  update() {
    // 距离超出时强制关闭（可选）
    if (this.player && this.autoCloseDistance > 0) {
      ...
      if (NPCInteract._TMP_A.length() > this.autoCloseDistance) {
        if (this._talkingLocal) {
          this._log(`距离>${this.autoCloseDistance}m，强制关闭对话与商店`);
          DialoguePanel.instance()?.close();
        }
        // ★ 无论当前是否对话，只要开着商店就关掉
        ShopWindow.closeIfOpen();
      }
    }
  }
  ...
  private _onTriggerLeave(e: any) {
    ...
    if (this._talkingLocal) {
      this._log('离开区域 → 关闭对话');
      DialoguePanel.instance()?.close(); // onClose 会复位 _talkingLocal
    }
    ShopWindow.closeIfOpen(); // ★ 同时关闭商店
  }

  private _onKeyDown(e: EventKeyboard) {
    if (e.keyCode !== this.interactKey) return; // 只响应指定键
  ...
  public startDialogue() {
    ...
    InteractHint.instance()?.hide();
    ShopContext.currentNPC = this.node;    // 记录当前正在对话的 NPC
